...
int strncasecmp(const char *s1, const char *s2, size_t n)
{
  ...
  if (*s1 != *s2 && n && !diff) { diff = tolower(*s1) - tolower(*s2); }
  return diff;
}
...
void _trim_string(char **str, size_t *len)
{
  ...
  while (*start && trimmed_len && isspace(*start)) { start++; trimmed_len--; }
  while (trimmed_len && isspace(start[trimmed_len-1])) { trimmed_len--; }
...
csv_value_t _field_to_value(csv_type_t field_type, char *field_str, size_t field_len)
{
  ...
  switch(field_type)
  {
    ...
    case string_type:
      ...
      if (value.data.string_value == NULL) { value.type = error_type; }
      break;
    case none_type:
      break;
    default:
  ...
  if (endptr && (endptr - field_str != field_len)) { value.type = error_type; }
...
char *_collect_field_str(char *line, char **field_str, size_t *field_len)
{
  ...
  *field_str = ptr;
  while (*ptr && *ptr != CSV_SEPARATOR) { ptr++; }
...
int _get_column_def_index(csv_context_t *context, char *header_str, int header_len)
{
  ...
  {
    if (header_len && !strncasecmp(context->column_defs[column_def_index].label, header_str, header_len))
    {
...
int _parse_headers(csv_context_t *context, char *line)
{
  ...
  while (*next)
  {
    ...
    context->column_def_index[column_count] = _get_column_def_index(context, header_str, header_len);
    column_count++;
  }
...
int _parse_values(csv_context_t *context, char *line)
{
  ...
  while (*line && column_index < context->num_columns)
  {
    ...
    if (column_def_index != _CSV_NO_DEF_INDEX)
    {
      ...
      if (field_value.type == error_type) { return CSV_ERROR; }
    }
    else
    {
...
int _update_row_cap(csv_context_t *context)
{
  ...
  {
    new_cap = context->row_cap > 0 ? context->row_cap * _CSV_ROW_CAP_MULTIPLIER : _CSV_ROW_CAP_INITIAL_SIZE ;
    new_rows = (csv_row_t *)realloc(context->rows, new_cap*sizeof(csv_row_t));
    if (!new_rows) { return CSV_ERROR; }
...
int get_csv_row_data(csv_context_t *context, size_t row_index, void *struct_ptr)
{
    ...
    if (row_index >= context->num_rows) { return CSV_ERROR; }
    for (size_t column_index = 0; column_index < context->num_columns; column_index++)
    {
        ...
        if (column_def_index != _CSV_NO_DEF_INDEX)
        {
            status = context->column_defs[column_def_index].setter(struct_ptr, context->rows[row_index][column_index]);
            if (status != CSV_OK) { break; }
        }
    }
