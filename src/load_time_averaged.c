...
int load_time_averaged_data(csv_context_t* context, char* filepath)
{
  ...
  // Set up columns
  status = status || def_csv_column(context, "time", double_type,
                                    set_dummy);
  ...
  status = status || def_csv_column(context, "door_time_to_open", double_type,
                                    set_door_time_to_open);
  status = status || def_csv_column(context, "leveling_time", double_type,
                                    set_leveling_time);
  ...
  status = status || def_csv_column(context, "num_cycle", double_type,
                                    set_num_cycles);
  status = status || def_csv_column(context, "flushing_discharge", double_type,
                                    set_flushing_discharge);
